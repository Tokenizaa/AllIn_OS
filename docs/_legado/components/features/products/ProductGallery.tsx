...
interface ProductGalleryProps {
  limit?: number; // Número máximo de produtos a serem exibidos (undefined para mostrar todos)
}
...
const ProductGalleryContent = ({ limit }: ProductGalleryProps) => {
  ...
  // Usar o novo contexto para carregar produtos
  const { products, loading, error } = useProducts();

  // Carregar produtos quando a lista de produtos mudar
  useEffect(() => {
    if (products.length > 0) {
      // Se limit for definido, limitar a exibição ao número especificado
      if (limit !== undefined) {
        setDisplayedProducts(products.slice(0, limit));
      } else {
        // Se limit for undefined, mostrar todos os produtos
  ...
  return (
    <>
      ...
      {/* Botão do Carrinho - Visível apenas em desktop */}
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {displayedProducts.map((product, index) => (
              <Card 
                key={index} 
                className="overflow-hidden border border-allin-orange/40 shadow-lg hover:shadow-xl transition-all duration-300 bg-allin-bg-light-1 dark:bg-allin-bg-dark-1 group animate-slide-up glass-card h-full flex flex-col dark:dark:bg-allin-bg-dark-3 dark:dark:border-allin-bg-dark-2"
          ...
          {/* Botão Ver Mais Produtos - só mostrar se houver limite */}
          {limit !== undefined && (
            <div className="text-center mt-12">
              <Button 
                variant="vibrant"
                onClick={() => navigate('/loja')}
                className="px-8 py-3 text-lg font-semibold transition-all duration-300 hover:scale-105 glass-button gap-2"
              >
                Ver Mais Produtos

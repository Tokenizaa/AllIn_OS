...
// Mock themes for different distributors
const PARTNER_THEMES: Record<string, {
  color: string;
  gradient: string;
  badgeBg: string;
  btnBg: string;
...
const DEFAULT_THEME = {
  ...
  bio: "Distribuidor oficial autorizado All-In Life. Comprometido com a disseminação de saúde integrativa de luxo e formação de times de vendas exponenciais.",
  quote: "Desenvolva sua melhor versão diária cuidando de suas células e gerando prosperidade."
};

function DistributorPage() {
  const { slug } = Route.useParams();
  const navigate = useNavigate();
  const { usersList, register } = useAuth();
  
  const sponsorSlug = slug.toLowerCase().trim();
  const theme = PARTNER_THEMES[sponsorSlug] || DEFAULT_THEME;

  // Let's find if there is an existing distributor with that slug in the userList
  ...
  return (
    <div className="min-h-screen bg-[#06080d] text-white selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      {/* HEADER NAV */}
      <nav id="landing-navbar" className="relative border-b border-border/10 bg-[#06080d]/80 backdrop-blur-md z-40 sticky top-0">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="h-8 w-8 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-400 flex items-center justify-center font-black text-black text-sm shadow-md">
              A
            </span>
          ...
          <div className="flex items-center gap-4 text-xs">
            <Link to="/loja/$slug" params={{ slug: sponsorSlug }} className="text-muted-foreground hover:text-white font-medium transition-colors">
              Loja
            </Link>
            <Link to="/seja-distribuidor/$slug" params={{ slug: sponsorSlug }} className="text-muted-foreground hover:text-white font-medium transition-colors">
              Recrutamento
            </Link>
            <Link to="/login" className="px-4 py-1.5 rounded-lg border border-border/40 hover:bg-background/20 font-medium transition-colors">
              Acessar Painel
            </Link>
          </div>
        </div>

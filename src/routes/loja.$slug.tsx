...
function DistributorStorePage() {
  ...
  return (
    <div className="min-h-screen bg-[#06080d] text-white selection:bg-emerald-500/30 overflow-x-hidden">
      ...
      {/* HEADER NAVBAR */}
      <nav id="store-navbar" className="relative border-b border-zinc-900 bg-[#06080d]/80 backdrop-blur-md z-40 sticky top-0">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link to="/$slug" params={{ slug: sponsorSlug }} className="flex items-center gap-2 cursor-pointer">
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-zinc-500 mr-1.5 font-mono">CARRINHO ({cart.reduce((ac, i) => ac + i.quantity, 0)})</span>
            <button
              onClick={() => {
                const el = document.getElementById("cart-drawer");
                if (el) el.scrollIntoView({ behavior: "smooth" });
              }}
              className="h-10 w-10 rounded-xl bg-background/50 border border-border flex items-center justify-center text-zinc-300 hover:text-white cursor-pointer relative"
            >
              <ShoppingBag className="h-4 w-4" />
              {cart.length > 0 && (
                <span className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-emerald-500 text-[9px] font-bold text-black flex items-center justify-center">
                  {cart.reduce((ac, i) => ac + i.quantity, 0)}
                </span>
              )}
            </button>
          </div>

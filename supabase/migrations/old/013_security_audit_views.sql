-- =====================================================================
-- ALLIN Sistema - Views de Segurança e Auditoria
...
LEFT JOIN public.pedidos_status ps ON ps.id = o.status_pedido_id
LEFT JOIN public.customers c ON c.id = o.customer_id
LEFT JOIN public.formas_pagamento fp ON fp.id = o.forma_pagamento_id
LEFT JOIN public.order_items oi ON oi.order_id = o.id
GROUP BY o.id, ps.nome, ps.cor, c.id_comprador, c.nome, c.email, fp.nome;
...
SELECT
    ...
    w.saldo_para_compra,
    w.total_recebido,
    COUNT(t.id) FILTER (WHERE t.transaction_type = 'CREDIT') AS creditos,
    COUNT(t.id) FILTER (WHERE t.transaction_type = 'DEBIT') AS debitos,
    COALESCE(SUM(t.amount) FILTER (WHERE t.transaction_type = 'CREDIT'), 0) AS total_creditado,
    COALESCE(SUM(t.amount) FILTER (WHERE t.transaction_type = 'DEBIT'), 0) AS total_debitado,
    w.created_at,
    w.updated_at
FROM public.wallets w
LEFT JOIN public.customers c ON c.id = w.customer_id
LEFT JOIN public.transactions t ON t.wallet_id = w.id
GROUP BY w.id, c.id_comprador, c.nome;

-- ---------------------------------------------------------------------
-- View: Network Tree View (view para árvore genealógica)
-- ---------------------------------------------------------------------
...
WITH RECURSIVE network_tree AS (
    ...
    WHERE c.id_comprador IN (
        SELECT DISTINCT parent_id FROM public.network_relationships
        WHERE parent_id NOT IN (SELECT child_id FROM public.network_relationships)
    )
    ...
    SELECT
        ...
        nr.parent_id,
        nt.level + 1,
        nt.path || c.id_comprador
    FROM public.customers c
    INNER JOIN public.network_relationships nr ON nr.child_id = c.id_comprador
    INNER JOIN network_tree nt ON nt.node_id = nr.parent_id
    WHERE NOT c.id_comprador = ANY(nt.path)
)
SELECT
    nt.node_id,
    nt.node_nome,
...
LEFT JOIN public.customer_network_metrics cnm ON cnm.customer_id = (SELECT id FROM public.customers WHERE id_comprador = nt.node_id)
ORDER BY nt.path;
...
SELECT
    ...
    p.payment_date,
    p.due_date,
    pm.nome AS method_nome,
    g.name AS gateway_nome
FROM public.payments p
LEFT JOIN public.orders o ON o.id = p.order_id
LEFT JOIN public.customers c ON c.id = p.customer_id
LEFT JOIN public.payment_methods pm ON pm.id = p.payment_method
LEFT JOIN public.gateways g ON g.id = (SELECT gateway_id FROM public.payment_attempts WHERE payment_id = p.id LIMIT 1);
...
SELECT
    ...
    (
        ...
        FROM (
            SELECT o.order_number, o.data_criacao, o.valor_total_pedido, ps.nome
            FROM public.orders o
            LEFT JOIN public.pedidos_status ps ON ps.id = o.status_pedido_id
